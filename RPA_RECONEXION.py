...
from utils.ots import lista_ots, excepcion_observa, grabar_logs, lista_ots_sin_hacer_tarea
from datetime import datetime
from bs4 import BeautifulSoup
from utils.variables import variables_config
...
URL = variables_config(53)
SALTO_URL = variables_config(54)
CONCURRENCY = 4
...
async def run_playwright_for_nrolote(nrolote, user, password):
    print("Iniciando RPA de Reconexión para nrolote:", nrolote)
    ...
    lista_ots_sin_hacer_tarea(nrolote)
        ...
        async with sem:
            ...
            try:
                ...
                await page1.evaluate(
                    """(fecha) => {
                        const input = document.querySelector("#baja_trabajo");
                        input.value = fecha;
                ...
                await loop.run_in_executor(None, grabar_logs, f'Cambiando la fecha de ejecucion inicial de la OT {ot}', nrolote, ot)
                guardar_fecha = page1.locator("//input[@type='button' and @id='btn_ejecutar' and @value='Ejecutar']")
                #page1.once("dialog", lambda dialog: dialog.accept()) # esto es para grabar la fecha
                ...
                await page1.locator("iframe[name=\"iframeObservaciones\"]").content_frame.locator("#v_new_observacion").fill(f"Reactivado por Telprime. NROLOTE: [{nrolote}]")
